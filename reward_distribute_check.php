...
$RewardDetails = $db->custom("SELECT * from mlm_reward_details where CONCAT(membershipId,reward_amount) NOT IN ( SELECT CONCAT(membership_id,amount) from mlm_ewallet where reason = 'Reward' and createdate = '$createdate') and reward_end_date >= '$createdate' and createdate < '$createdate'");

if($RewardDetails->num_rows >= 1){

    $result = array();
    while($row = $RewardDetails->fetch_assoc()){
        $result[] = $row;
    };
    
    foreach($result as $rewardRow){
