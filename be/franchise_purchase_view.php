...
@$fpurchaseid = $validation->input_validate($_GET['fpurchaseid']);
...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Franchise <!--<?php if($_SESSION['per_write'] == "1") { ?><a href="purchase_form.php?mode=insert" class="btn btn-default btn-sm button">Add New</a><?php } ?>--></h1>
	</div>
</div>

<form name="form_actions" method="POST" action="franchise_purchase_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
<div class="row">
	<div class="col-sm-12 mb-0">
		<div class="form-inline">
			<select NAME="bulk_actions" CLASS="form-control mb_inline mb-2" >
				...
				<!-- <option VALUE="paid">Status to Paid</option>
				<option VALUE="unpaid">Status to Unpaid</option> -->
			</select>
			...
			<a href="franchise_purchase_view.php" class="btn btn-default mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
		</div>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	<tr>
		...
		<th class="<?php echo $th_sort1." ".$th_order_cls1; ?>"><a href="franchise_purchase_view.php?orderby=refno_custom&order=<?php echo $th_order1; echo $url_parameters; ?>"><span>Order ID</span> <span class="sorting-indicator"></span></a></th>
		
		<th>Price</th>
		<th>Customer Name</th>
		<th>Order Status</th>
		<th>Status</th>
		<th class="<?php echo $th_sort3." ".$th_order_cls3; ?>"><a href="franchise_purchase_view.php?orderby=createdate&order=<?php echo $th_order3.''.$url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	if($data['num_rows'] >= 1)
	{
		...
		{
			$productid = $purchaseRow['fpurchaseid'];
			$productQueryResult = $db->view("title", "rb_franchise_purchase", "fpurchaseid", "and fpurchaseid='{$fpurchaseid}'");
		...
		<tr class="text-center has-row-actions">
			...
			<td data-label="Order ID - ">
				<a href="franchise_purchase_form.php?mode=edit&fpurchaseid=<?php echo $validation->db_field_validate($purchaseRow['fpurchaseid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($purchaseRow['refno_custom']); ?></a>
				
				<div class="row row-actions">
					<div class="col-sm-12">
						<?php if($validation->db_field_validate($purchaseRow['order_status']) != 'cancelled'){?>
							<?php if($_SESSION['per_update'] == "1") { ?>
								<a href="franchise_purchase_form.php?mode=edit&fpurchaseid=<?php echo $validation->db_field_validate($purchaseRow['fpurchaseid']); ?>">Edit</a>
								| 
							<?php } ?>
						<?php }?>
						<?php if($_SESSION['per_delete'] == "1") { ?>
							<a href="franchise_purchase_actions.php?q=del&refno_custom=<?php echo $validation->db_field_validate($purchaseRow['refno_custom']); ?>" onClick="return del();" class="delete">Delete</a>
			...
			<td data-label="Price - ">&#8377;  <?php echo $validation->price_format($purchaseRow['totalPrice']); ?></td>
			<td data-label="Customer Name - "><?php echo $validation->db_field_validate($purchaseRow['billing_first_name']." ".$purchaseRow['billing_last_name']); ?></td>
			<td data-label="Tracking Status - "><font color="<?php if($purchaseRow['order_status'] == "fulfilled") { echo "green"; } else { echo "red"; } ?>"><?php echo $validation->db_field_validate(ucfirst($purchaseRow['order_status'])); ?></font></td>
			<td data-label="Status - "><font color="<?php if($purchaseRow['status'] == "paid") { echo "green"; } else { echo "red"; } ?>"><?php echo $validation->db_field_validate(ucfirst($purchaseRow['status'])); ?></font></td>
