	...
	if($challengeQuery['num_rows']>=1){
		...
		if($sponsorResultRow['challenge_id'] <= '0'){
			...
			if($checkMemberQuery['num_rows'] >= $members){
				...
				$reward = "10% company revenue distribution!"; 
		...
		if($dataResult['num_rows']>=1)
		{
			foreach($dataResult['result'] as $memberRow)
			{
				...
				if ($rewardResult['num_rows'] >= 1) {
					...
					echo $amount . PHP_EOL . $memberRow['current_member'] . PHP_EOL . $members . PHP_EOL . $memberRow['direct_member'] . PHP_EOL .$direct_members . PHP_EOL;
				
					if ($amount != "") {
						if($memberRow['current_member'] >= $members && $memberRow['direct_member'] >= $direct_members){
							$refno = substr(md5(rand(1, 99999)), 0, 10);
