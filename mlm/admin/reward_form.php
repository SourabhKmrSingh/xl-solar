		...
		<div ID="page-wrapper">
			<div CLASS="container-fluid">
				...
				<form name="dataform" method="post" class="form-group" action="<?php
																				switch ($mode) {
																					case "insert":
																						echo "reward_form_inter.php?mode=$mode";
																						break;

																					case "edit":
																						echo "reward_form_inter.php?mode=$mode&rewardid=$rewardid";
																						break;

																					default:
																						echo "reward_form_inter.php";
																				}
					...
					<div class="form-rows-custom mt-3">
								...
								<select NAME="planid" CLASS="form-control" ID="planid" required>
									...
									?>
										<option VALUE="<?php echo $validation->db_field_validate($planRow['planid']); ?>" <?php if ($mode == 'edit') {
																																if ($planRow['planid'] == $rewardRow['planid']) echo "selected";
						...
						<div class="row mb-3">
							<div class="col-sm-3">
								<label for="time_period">Time Period (In days)</label>
							</div>
							<div class="col-sm-9">
								<input type="number" name="time_period" id="time_period" class="form-control" value="<?php if ($mode == 'edit') echo $validation->db_field_validate($rewardRow['time_period']); ?>" />
							</div>
						</div>

						<div class="row mb-3">
							<div class="col-sm-3">
								<label for="direct_members">Direct Members</label>
							</div>
							<div class="col-sm-9">
								<input type="text" name="direct_members" id="direct_members" class="form-control" value="<?php if ($mode == 'edit') echo $validation->db_field_validate($rewardRow['direct_members']); ?>" />
							</div>
						</div>
								...
								<?php if ($mode == 'edit' and $rewardRow['imgName'] != "") { ?>
									<div class="mt-2 links">
										<img src="<?php echo IMG_THUMB_LOC;
													echo $validation->db_field_validate($rewardRow['imgName']); ?>" title="<?php echo $validation->db_field_validate($rewardRow['imgName']); ?>" class="img-responsive mh-51" /><br>
										<a href="<?php echo IMG_MAIN_LOC;
													echo $validation->db_field_validate($rewardRow['imgName']); ?>" target="_blank">Click to Download</a> | <a href="reward_form.php?mode=edit&rewardid=<?php echo $rewardid; ?>&q=imgdel" onClick="return del();">Delete</a>
									</div>
								<?php } ?>
								...
								<?php if ($mode == 'edit' and $rewardRow['fileName'] != "") { ?>
									<div class="mt-2 links">
										<a href="<?php echo FILE_LOC;
													echo $validation->db_field_validate($rewardRow['fileName']); ?>" target="_blank">Click to Download</a> | <a href="reward_form.php?mode=edit&rewardid=<?php echo $rewardid; ?>&q=filedel" onClick="return del();">Delete</a>
									</div>
								<?php } ?>
						...
						<div class="row mb-3">
							...
							<div class="col-sm-9">
								<input type="checkbox" name="priority" id="priority" <?php if ($mode == 'edit') {
																							if ($rewardRow['priority'] == "1") echo "checked";
																						} ?> />
							</div>
						</div>
							...
							<div class="col-sm-9">
								<select name="status" id="status" class="form-control" required>
									<option value="active" <?php if ($mode == 'edit') {
																if ($validation->db_field_validate($rewardRow['status']) == "active") echo "selected";
															} ?>>Active</option>
									<option value="inactive" <?php if ($mode == 'edit') {
																	if ($validation->db_field_validate($rewardRow['status']) == "inactive") echo "selected";
																} ?>>Inactive</option>
								</select>
							</div>
						</div>
	...
	<div ID="image_model" CLASS="modal">
		<div CLASS="modal-content">
			...
			<div STYLE="background:; padding:3%;">
				<p styl= "text-align:center">Select/Upload files from your local machine to server.</p>
				<div ID="drop-area">
