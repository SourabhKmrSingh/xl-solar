...
if($q == "del")
{
	...
	$delresult = $media->multiple_filedeletion('mlm_rewards_history', 'historyid', $historyid, 'imgName', IMG_MAIN_LOC, IMG_THUMB_LOC);
	$delresult2 = $media->filedeletion('mlm_rewards_history', 'historyid', $historyid, 'fileName', FILE_LOC);

	$challengehistoryQueryResult = $db->delete("mlm_rewards_history", array('historyid'=>$historyid));
	if(!$challengehistoryQueryResult)
	{
		$_SESSION['error_msg'] = "Error Occurred. Please try again!!!";
		header("Location: reward_history_view.php");
		exit();
	}
	...
	if(isset($del_items) and $del_items != "")
	{
		foreach($del_items as $id)
		{
			if($bulk_actions == "delete")
			{
				...
				$delresult = $media->filedeletion('mlm_rewards_history', 'historyid', $id, 'imgName', IMG_MAIN_LOC, IMG_THUMB_LOC);
				$delresult2 = $media->filedeletion('mlm_rewards_history', 'historyid', $id, 'fileName', FILE_LOC);
			}
		...
		if($bulk_actions == "delete")
		{
			$challengehistoryQueryResult = $db->custom("DELETE from mlm_rewards_history where FIND_IN_SET(`historyid`, '$historyids')");
			if(!$challengehistoryQueryResult)
			{
				$_SESSION['error_msg'] = "Error Occurred. Please try again!!!";
				header("Location: reward_history_view.php");
		...
		{
			$challengehistoryQueryResult = $db->custom("UPDATE mlm_rewards_history SET status='$bulk_actions' where FIND_IN_SET(`historyid`, '$historyids')");
			if(!$challengehistoryQueryResult)
			{
...
else
{
	...
	header("Location: reward_history_view.php?$fields_string");
	exit();
}
?>