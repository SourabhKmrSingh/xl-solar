...
$percentage = $validation->input_validate($_POST['percentage']);
if($percentage=='')
{
	$percentage = 0;
}
...
// $dupresult = $db->check_duplicates('mlm_repurchase', 'repurchaseid', $repurchaseid, 'title_id', strtolower($title_id), $mode);
// if($dupresult >= 1)
// {
...
$fields = array('planid'=>$planid, 'title'=>$title, 'title_id'=>$title_id, 'percentage'=>$percentage, 'order_custom'=>$order_custom, 'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$levelQueryResult = $db->insert("mlm_repurchase", $fields);
	if(!$levelQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$levelQueryResult = $db->update("mlm_repurchase", $fields, array('repurchaseid'=>$repurchaseid));
	if(!$levelQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: repurchase_view.php$search_filter");
	exit();
}
?>