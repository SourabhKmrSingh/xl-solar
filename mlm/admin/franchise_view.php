...
if ($orderby != "" and $order != "") {
	...
	$orderby_final = "franchiseid desc";
}
	...
	<div ID="wrapper">
		...
		<div ID="page-wrapper">
			<div CLASS="container-fluid">
				<div CLASS="row">
					<div CLASS="col-lg-12">
						<h1 CLASS="page-header">Franchise <?php if ($_SESSION['per_write'] == "1") { ?><a href="franchise_form.php?mode=insert" class="btn btn-default btn-sm button">Add New</a><?php } ?></h1>
					</div>
				</div>

				<form name="form_actions" method="POST" action="franchise_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
					<div class="row">
						<div class="col-sm-12 mb-0">
							<div class="form-inline">
								...
								<a href="franchise_view.php" class="btn btn-default mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
							</div>
						</div>
					</div>
						...
						<table class="table table-striped table-view" cellspacing="0" width="100%">
							<thead>
								<tr>
									...
									<th class="<?php echo $th_sort1 . " " . $th_order_cls1; ?>"><a href="franchise_view.php?orderby=title&order=<?php echo $th_order1;
							...
							<tbody>
								...
								?>
										<tr class="text-center has-row-actions">
											<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($franchiseRow['franchiseid']); ?>" /></td>
											<td data-label="Title - ">
												<a href="franchise_form.php?mode=edit&franchiseid=<?php echo $validation->db_field_validate($franchiseRow['franchiseid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($franchiseRow['title']); ?></a>

												<div class="row row-actions">
													<div class="col-sm-12">
														<?php if ($_SESSION['per_update'] == "1") { ?>
															<a href="franchise_form.php?mode=edit&franchiseid=<?php echo $validation->db_field_validate($franchiseRow['franchiseid']); ?>">Edit</a>
															|
														<?php } ?>
														<?php if ($_SESSION['per_delete'] == "1") { ?>
															<a href="franchise_actions.php?q=del&franchiseid=<?php echo $validation->db_field_validate($franchiseRow['franchiseid']); ?>" onClick="return del();" class="delete">Delete</a>
														<?php } ?>
													</div>
												</div>
											</td>
											<td data-label="Author - "><a href="franchise_view.php?userid=<?php echo $userid; ?>"><?php echo $validation->db_field_validate($userRow['display_name']); ?></a></td>
											<td data-label="Pins - "><?php echo $franchiseRow['total_pins']; ?></td>
											<td data-label="free - "><?php echo $franchiseRow['free']; ?></td>
											<td data-label="Plan - "><a href="franchise_view.php?planid=<?php echo $planid; ?>"><?php echo $validation->db_field_validate($planRow['title']); ?></a></td>
