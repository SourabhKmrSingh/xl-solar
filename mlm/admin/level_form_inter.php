...
$commission = $validation->input_validate($_POST['commission']);
if($commission=='')
{
	$commission = 0;
}
...
// $dupresult = $db->check_duplicates('mlm_levels', 'levelid', $levelid, 'title_id', strtolower($title_id), $mode);
// if($dupresult >= 1)
// {
...
$fields = array('planid'=>$planid, 'title'=>$title, 'title_id'=>$title_id, 'percentage'=>$percentage, 'commission'=>$commission, 'order_custom'=>$order_custom, 'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$levelQueryResult = $db->insert("mlm_levels", $fields);
	if(!$levelQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$levelQueryResult = $db->update("mlm_levels", $fields, array('levelid'=>$levelid));
	if(!$levelQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: level_view.php$search_filter");
	exit();
}
?>