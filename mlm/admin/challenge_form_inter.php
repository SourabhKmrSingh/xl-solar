...
if($time_period == '' && $expiry_date == ''){
	$_SESSION['error_msg'] = "Please enter expiry date or time period!";
		header("Location: challenge_view.php");
		exit();
}
if($time_period=='')
...
$reward = $validation->input_validate($_POST['reward']);
...
// $dupresult = $db->check_duplicates('mlm_challenges', 'challengeid', $challengeid, 'title_id', strtolower($title_id), $mode);
// if($dupresult >= 1)
// {
...
$fields = array('planid'=>$planid, 'title'=>$title, 'title_id'=>$title_id, 'time_period'=>$time_period, 'members'=>$members, 'reward'=>$reward, 'order_custom'=>$order_custom,'expiry_date'=>$expiry_date,'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$challengeQueryResult = $db->insert("mlm_challenges", $fields);
...
else if($mode == "edit")
{
	...
	$challengeQueryResult = $db->update("mlm_challenges", $fields, array('challengeid'=>$challengeid));
	if(!$challengeQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: challenge_view.php$search_filter");
	exit();
}
?>