...
$where_query = " and membership_id='SL001'";
if ($regid != "") {
	...
	<div ID="wrapper">
		...
		<div ID="page-wrapper">
			<div CLASS="container-fluid">
				<div CLASS="row">
					<div CLASS="col-lg-12">
						<h1 CLASS="page-header">Activate Account <?php if ($_SESSION['per_write'] == "1") { ?><a href="active_account_form.php?mode=insert&membershipid=<?php echo $data['result'][0]['membership_id'];?>" class="btn btn-default btn-sm button">Activate Account</a><?php } ?></h1>
					</div>
				</div>

				<form name="form_actions" method="POST" action="active_account_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
					<div class="row">
						<div class="col-sm-12 mb-0">
							<div class="form-inline">
								...
								<a href="active_account_view.php" class="btn btn-default mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
							</div>
						</div>
					</div>
						...
						<table class="table table-striped table-view" cellspacing="0" width="100%">
							<thead>
								<tr>
									...
									<th>Username</th>
									<th>Status</th>
									<th class="<?php echo $th_sort3 . " " . $th_order_cls3; ?>"><a href="active_account_view.php?orderby=createdate&order=<?php echo $th_order3 . '' . $url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
								</tr>
							</thead>
							<tbody>
								<?php
								...
								?>
										<tr class="text-center has-row-actions">
											<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($franchiseRow['pinid']); ?>" /></td>
											<td data-label="Pins - "><?php echo $franchiseRow['pin']; ?>
												<div class="row row-actions">
													<div class="col-sm-12">
														<?php if($_SESSION['per_delete'] == "1") { ?>
															<a href="active_account_actions.php?q=del&pinid=<?php echo $validation->db_field_validate($franchiseRow['pinid']); ?>" onClick="return del();" class="delete">Delete</a>
														<?php } ?>
													</div>
												</div>
											</td>
											<td data-label="Pins - "><?php echo $franchiseRow['membership_id']; ?></td>
